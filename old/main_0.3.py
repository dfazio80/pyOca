#  pyOca 0.3 - una versión del juego de la oca en python
...
class cTablero:
    ...
    def __init__(self):
        ...
        self.coordY = [515,515,515,515,515,515,515,515,515,485,445,#10
                         ...
                         320,320,320]
        try:
            self.img = pygame.image.load(os.path.join(img_folder,'tablero_oca.png')).convert()
        except:
    ...
    def __init__(self):
        ...
        try:
            self.listimg = [pygame.Surface((0,0)), 
                            pygame.image.load(os.path.join(img_folder,'dado1.png')).convert_alpha(),
                            pygame.image.load(os.path.join(img_folder,'dado2.png')).convert_alpha(),
                            pygame.image.load(os.path.join(img_folder,'dado3.png')).convert_alpha(),
                            pygame.image.load(os.path.join(img_folder,'dado4.png')).convert_alpha(),
                            pygame.image.load(os.path.join(img_folder,'dado5.png')).convert_alpha(),
                            pygame.image.load(os.path.join(img_folder,'dado6.png')).convert_alpha(),
                            pygame.image.load(os.path.join(img_folder,'gana.png')).convert_alpha(),  # posic 7: cara ganador
                            pygame.image.load(os.path.join(img_folder,'pierde.png')).convert_alpha()]  # posic 8: cara perdedor
...
class cJugador:
    ...
    def __init__(self, color, primerPos):
        ...
        self.pierdeTurno = 0
        try:
            self.img = pygame.image.load(os.path.join(img_folder,'ficha_'+color+'.png')).convert()
        except:
...
def casilla_libre(jugAct, jugEsp, valor):
    #jugAct: jugador actual (el que tiro el dado), #jugEsp: jugador en espera
    # valor es cuantas casillas debo retroceder en caso que esté ocupada
    if (jugAct.posic == jugEsp.posic): #Cayo en la misma casilla que el otro jugador, debe retroceder
        ...
        jugAct.mover(valor, avanza = False)
...
def evalua_casilla(jugAct, jugEsp):
    ...
    if jugAct.posic in POS_OCA: #Oca
        ...
        if casilla_libre(jugAct, jugEsp, POS_POSADA - posAnt): #verifico que no haya nadie en la posada
        ...
        pygame.time.wait(800)
        if casilla_libre(jugAct, jugEsp, dado.valor): #verifico que no haya nadie en la posada
            jugAct.pierdeTurno = 2
            cJugador.turno = cambia_turno()
        ...
        pygame.time.wait(800)
        if casilla_libre(jugAct, jugEsp, dado.valor): #verifico que no haya nadie en el pozo
        ...
        if casilla_libre(jugAct, jugEsp, 1): #verifico que no haya nadie en la 30, si hay alguien en la 30, va a la 29            
            cJugador.turno = cambia_turno()

    elif jugAct.posic == POS_CARCEL: #Carcel
        eventosTxt.cambiaTxt('carcel', NEGRO)
        pygame.time.wait(800)
        if casilla_libre(jugAct, jugEsp, dado.valor): #verifico que no haya nadie en la carcel
        ...
        if casilla_libre(jugAct, jugEsp, dado.valor): #verifico que no haya nadie en la casilla
            cJugador.turno = cambia_turno()

def hay_ganador(j1, j2):
    if j1.posic == POS_LLEGA:
        eventosTxt.cambiaTxt('llega', NEGRO)
        turnoTxt.cambiaTxt('vacio', NEGRO)
...
def main():
    ...
    while True:
        define_primero()
        dibuja_pantalla()
        while not hay_ganador(jugador1, jugador2): # loop principal
...
icono = pygame.image.load(os.path.join(img_folder,'icono.png')).convert()
pygame.display.set_icon(icono)
pygame.display.set_caption('pyOca 0.3 - El juego de la oca')
...
# genero dos dados temporales para la primer tirada y para mostrar las caritas
dadoA = cDado()
dadoB = cDado()
