#  pyOca 0.6 - juego de la oca hecho con python y pygame
...
class cTablero:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.cuak = pygame.mixer.Sound(os.path.join(data_folder,'s_cuak.ogg'))
            self.gana = pygame.mixer.Sound(os.path.join(data_folder,'s_gana.ogg'))
        except:
            print ("Error: No se pueden cargar los archivos de imagen y sonido del tablero")
            fin_programa()
...
class cDado:
    ...
    def __init__(self):
        ...
        try:
            ...
            print ("Error: No se pueden cargar los archivos de imagen y sonido de los dados")
            fin_programa()
    def tirar(self):
...
class cEventTexto:
    def __init__(self, txt, pos, color):
        ...
        self.texto = fuenteChica.render (self.dictTxt[txt], True, color)
        self.posXY = pos
...
def evalua_casilla(jugAct, jugEsp):
    ...
    if jugAct.posic in POS_OCA: #Oca
        ...
        if casilla_libre(jugAct, jugEsp): #verifico que no haya nadie en la posada
            ...
            jugEsp.pierdeTurno = 0 #blanqueo los turnos del otro jugador
            cJugador.turno = cambia_turno()
        ...
        pygame.time.wait(MSWAIT)
        jugAct.posic = 1 # En este caso no hago animacion porque es muy larga y puede colgarse
        jugAct.posX = tablero.coordX[jugAct.posic]
        jugAct.posY = tablero.coordY[jugAct.posic]
        dibuja_pantalla()
        cJugador.turno = cambia_turno()
...
def muestra_creditos():
    ...
    subTxt1 = fuenteChica.render ('El clasico juego de tablero en python (y pygame)', True, ROJO)
    ...
    subTxt4 = fuenteChica.render ('Graficos: http://openclipart.org/', True, ROJO)
    subTxt5 = fuenteChica.render ('Sonidos: http://freesound.org/', True, ROJO)
    display.blit(tituloTxt,(20,220))
    display.blit(subTxt1,(20,270))
    display.blit(subTxt2,(20,300))
    display.blit(subTxt3,(20,330))
    display.blit(subTxt4,(20,360))
    display.blit(subTxt5,(20,390))
    pygame.display.update()
...
def crea_objetos():
    global tablero, dado, jugador1, jugador2, eventosTxt, turnoTxt, dadoA, dadoB
    tablero = cTablero()
    dado = cDado()
    ...
    while True: #loop infinito
        ...
        while not hay_ganador(jugador1, jugador2): # loop principal del juego. Termina si hay un ganador
            for event in pygame.event.get(): # eventos de pygame -- deteccion de input de teclado
                ...
                if event.type == pygame.MOUSEBUTTONDOWN and event.button == LEFT:
                    muestra_creditos()
        # loop que espera si se pulsa la tecla enter para empezar un nuevo juego
        juegadeNuevo = False
...
LEFT = 1 #para el click de mouse izquierdo
# casillas especiales
...
fuenteChica = pygame.font.SysFont('Verdana', 18)
fuenteGrande = pygame.font.SysFont('Verdana', 24)
# inicio ventana
